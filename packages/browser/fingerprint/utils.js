...
function overrideIntlAPI(language) {
  try {
    ...
    Intl.DateTimeFormat = function (locales, options) {
      return new originalDateTimeFormat(locales ?? language, options);
    };

    // Copy the prototype so instanceof checks still work
    Intl.DateTimeFormat.prototype = originalDateTimeFormat.prototype;

    // Override Intl.NumberFormat constructor
    Intl.NumberFormat = function (locales, options) {
      return new originalNumberFormat(locales ?? language, options);
    };

    // Copy the prototype so instanceof checks still work
    Intl.NumberFormat.prototype = originalNumberFormat.prototype;

    // Extend this pattern to other Intl constructors as needed
  } catch (e) {
    console.warn("Failed to override Intl API:", e);
  }
}
